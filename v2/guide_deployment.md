...
Ordered chain (oldest → newest); the current head is `b1c2d3e4f5a6`. Run `flask --app app db upgrade` to apply everything up to the head — you do not apply these individually.
...
| 28 | `d3e4f5a6b7c8` | Adds the `voucher_batches` and `voucher_codes` tables (codes stored only as an HMAC; at most one code per participant) (PR #442). |
| 29 | `b1c2d3e4f5a6` | Adds `conversations.admin_notes` (nullable text; private organizer/global-admin notes, never exported or shown to participants) (PR #349). **Current head.** |

Verify the live head with `flask --app app db current`; confirm it matches `b1c2d3e4f5a6` after deploying. When new migrations land, append them here. `flask --app app db heads` must print exactly one revision; if it prints two, parallel branches have each added a migration and a merge revision (`flask --app app db merge -m "..." <head1> <head2>`) is needed before deploying.
