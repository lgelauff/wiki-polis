...
Ordered chain (oldest → newest); the current head is `c1d2e3f4a5b6`. Run `flask --app app db upgrade` to apply everything up to the head — you do not apply these individually.
...
| 27 | `c1d2e3f4a5b6` | **Merge revision** of `b0c1d2e3f4a5` + `6a7090c4c5d7` — no schema change (PR #439). **Current head.** |

Verify the live head with `flask --app app db current`; confirm it matches `c1d2e3f4a5b6` after deploying. When new migrations land, append them here. `flask --app app db heads` must print exactly one revision; if it prints two, parallel branches have each added a migration and a merge revision (`flask --app app db merge -m "..." <head1> <head2>`) is needed before deploying.
