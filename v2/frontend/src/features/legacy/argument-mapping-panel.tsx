...
/** Each side's copy is its own message rather than one message with the side interpolated:
 *  "for-argument" and "against-argument" inflect differently in many languages. Written out
...
function Contribution({slug, csrfToken, featuredId, side, value}: {
  ...
  const text = sideMessages(msg, side);
  useEffect(() => {
  ...
  return (
    <div className="contribute-wrapper" data-state={state} data-side={side} data-fs={featuredId}>
      ...
      <button className="at-direct-skip contribute-direct-skip" type="button" disabled={skip.isPending} onClick={() => skip.mutate()}>{msg('conv-arg-nothing')}</button>
      ...
      <div className="at-contribute at-contribute--skipped contribute-skipped">
        ...
        {msg('conv-arg-skipped')}
        <button type="button" className="at-link contribute-changemind" onClick={compose}>{msg('conv-arg-changemind')}</button>
      </div>
      {(submit.error || skip.error) && <p className="muted" role="alert">{msg('conv-arg-save-error')}</p>}
    </div>
  );
}
      ...
      <div className="at-card-body argument-body">
        ...
        <div className="at-card-meta argument-meta">
          <span className="at-star argument-importance">
            ...
            <strong className="importance-count" aria-hidden="true">{importance}</strong><span className="sr-only">{msg('conv-arg-importance-votes', importance)}</span>
          </span>
          {item.own && <span className="at-yours argument-yours">{msg('conv-arg-yours')}</span>}
          {item.hidden && <span className="at-hidden-badge argument-hidden-badge">{msg('conv-arg-hidden')}</span>}
          {item.capabilities.flag && <LegacyContentFlag slug={slug} target={{contentType: 'argument', targetId: item.id}} csrfToken={csrfToken} />}
          {item.capabilities.moderate && <span style={{display: 'inline-flex', gap: 4, marginLeft: 'auto'}}><button type="button" className={`btn-small${item.hidden ? '' : ' btn-warn'}`} disabled={moderate.isPending} onClick={(event) => { event.stopPropagation(); moderate.mutate(); }}>{item.hidden ? msg('conv-arg-unhide') : msg('conv-arg-hide')}</button></span>}
        </div>
      </div>
    </div>
  );
...
function ArgumentColumn({mapping, slug, csrfToken, card, side}: {
  ...
  return (
    <div className={`at-col at-col--${copy.className}`}>
      ...
      {card.contributionsComplete && !voteReady && <div className="at-volnote" id={`volnote-${side}-${card.id}`}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 9v4M12 17h.01" /><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /></svg><span>{side === 'pro'
        ? msg('conv-arg-volnote-for', value.prioritization.selectionBudget, value.arguments.length)
        : msg('conv-arg-volnote-against', value.prioritization.selectionBudget, value.arguments.length)}</span></div>}
    </div>
  );
}
    ...
    <div className={`at-panel arg-block${index !== currentIndex ? ' arg-block--hidden' : ''}${complete ? ' at-panel--done' : ''}${expanded ? ' at-panel--expanded' : ''}`} id={`fs-${card.id}`} tabIndex={-1} data-index={index} aria-hidden={index !== currentIndex || undefined}>
      <div className="at-head" {...(complete ? {role: 'button', tabIndex: 0, 'aria-expanded': expanded, 'aria-label': expanded ? msg('conv-arg-panel-collapse-aria') : msg('conv-arg-panel-expand-aria'), onClick: () => setExpanded(!expanded)} : {})}>
        <div className="at-head-main">
          <LegacyContentFlag slug={slug} target={{contentType: 'statement', targetId: card.statement.id}} csrfToken={csrfToken} corner />
          <div className="at-featured-label" aria-hidden="true"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="m12 2 3 7 7 .5-5.5 4.5 2 7-6.5-4-6.5 4 2-7L2 9.5 9 9z" /></svg>{msg('conv-arg-featured-label')}</div>
          <p className="at-stmt">{card.statement.text}</p>
        </div>
        <div className="at-steps" id={`steps-${card.id}`}>
          <div className={`at-step ${card.contributionsComplete ? 'at-step--done' : 'at-step--active'}`} id={`step1-${card.id}`}><span className="at-step-num">{card.contributionsComplete ? '✓' : '1'}</span><div><div className="at-step-name">{msg('conv-arg-step1-name')}</div><div className="at-step-sub" id={`step1-sub-${card.id}`}>{stepOneSub}</div></div></div>
          <div className={`at-step ${card.contributionsComplete ? 'at-step--active' : 'at-step--locked'}`} id={`step2-${card.id}`}><span className="at-step-num">{card.contributionsComplete ? '2' : '🔒'}</span><div><div className="at-step-name">{msg('conv-arg-step2-name')}</div><div className="at-step-sub" id={`step2-sub-${card.id}`}>{card.contributionsComplete ? msg('conv-arg-step2-sub', pro.selectedCount, pro.selectionBudget, con.selectedCount, con.selectionBudget) : msg('conv-arg-step2-locked')}</div></div></div>
        </div>
      ...
      <div className="at-nav">
        <button className="at-navbtn at-navbtn--prev" type="button" disabled={index === 0} aria-label={msg('conv-nav-prev-aria')} onClick={() => setIndex(index - 1)}>{msg('conv-nav-prev')}</button>
        ...
        <button className="at-navbtn at-navbtn--next" type="button" disabled={index === mapping.featuredStatements.length - 1} aria-label={msg('conv-nav-next-aria')} onClick={() => setIndex(index + 1)}>{msg('conv-nav-next')}</button>
      </div>
    </div>
  );
}
...
export function LegacyArgumentMappingPanel({slug, csrfToken}: {slug: string; csrfToken: string}) {
  ...
  if (data.featuredStatements.length === 0) return <div className="landing-section"><p className="muted">{msg('conv-arg-none')}</p></div>;
  return (
    <>
      ...
      <div className="at-complete" id="at-complete" hidden={!data.progress.allDone}><div className="interlude"><span className="interlude-glyph"><svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M20 6 9 17l-5-5" /></svg></span><h2 className="interlude-title">{msg('conv-arg-complete-title')}</h2><p className="interlude-sub">{msg('conv-arg-complete-sub')}</p></div></div>
