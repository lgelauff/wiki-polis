...
- **Entry.** Case, spaces and hyphens are ignored; nothing else is rewritten, and any code shape is looked up, since imported codes can be anything from 5 to 64 letters and digits. A wrong, revoked, expired-unused or in-flight code gets one indistinguishable "not valid" answer; when the typed code contains I, L, O or U (which generated codes never use) that answer adds a hint about 1 and 0. A redeemed code is not "used up": it keeps signing its holder back in.
- **Guessing.** Only wrong codes are counted: 10 a minute per browser session and 60 a minute per conversation, neither keyed on the client address. Over either, entry answers `voucher-throttled` (429 with `Retry-After`, "wait N seconds") until the minute passes; no code is locked or invalidated. Exhausting the conversation's budget is logged; alerting the organizers needs the organizer screens. After a code is accepted the browser goes to `/c/<slug>`, so the code leaves the address bar, and the usual join step follows (pseudonym and consent, without email or talk-page notifications).
